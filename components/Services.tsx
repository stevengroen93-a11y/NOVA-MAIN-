      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service) => {
            const Icon = service.icon;
            return (
              <div key={service.title} className="flex gap-4 p-6 rounded-2xl bg-orange-50/50 border border-orange-100/60 hover:border-orange-200 hover:bg-orange-50 transition-colors group">
                <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${service.gradient} flex items-center justify-center flex-shrink-0 shadow-sm mt-0.5`}>
                  <Icon className="w-5 h-5 text-white" />
                </div>
                <div>
                  <h3 className="text-[#0a0a0a] font-bold text-base mb-1.5">{service.title}</h3>
                  <p className="text-gray-500 text-sm leading-relaxed mb-3">{service.description}</p>
